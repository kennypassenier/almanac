...
| Field | Value |
|---|---|
| Current phase | done — **v4.0.4 tagged and signed 2026-09-10** (chassis-rs 2.0.0, `a2e0c32`): `Client::adopted(...)` replaces the struct literal (`Client` is `#[non_exhaustive]`), the Sources page's own calendar column renamed **Calendar name** (the kit auto-renders a column per declared client-issue-form field), and the full scaffold sync — static musl build on `gcr.io/distroless/static`, CI back to every-branch triggers, kp-themes 5.1.0, `tests/kit_smoke.rs` added — with `.githooks/commit-msg` and `.claude/hooks/check-commit.sh` hand-restored right after, since chassis-rs's vendored scaffold still predates Almanac's 2026-09-10 ID-scheme rollout. v4.0.3 was superseded unsigned and never installed anywhere. 4.0.2 (chassis-rs 1.7.0) still live on CT 112 until the homelab's supervised update runs. |
| Last completed gate | scaffold-sync decision (2026-09-10): "Dichten" — close the scaffold gap now rather than defer it; release chain run by Claude through the unsigned GitHub release, `scripts/sign-release.sh v4.0.4` run by Kenny |
| Next gate | Homelab Rust's supervised `almanac update` on CT 112 (4.0.2 → 4.0.4); a chassis-rs correction is queued for after the retro: `run()` in `chassis-cli` uses `Command::output()` for the `scripts/sign-release.sh` step, which captures stdout/stderr and hides minisign's password prompt (stdin still works, so the release did not actually block — just looked like it had) |
