...
| Next gate | none pressing — CT 112 has not moved to Debian 13 yet (CT 109 has, today); the static musl build makes that irrelevant whenever it happens. A chassis-rs correction is queued for after the retro: `run()` in `chassis-cli` uses `Command::output()` for the `scripts/sign-release.sh` step, which captures stdout/stderr and hides minisign's password prompt |
