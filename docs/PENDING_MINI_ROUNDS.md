...
| `backoff` unmaintained (RUSTSEC-2025-0012) | Replace the `backoff` crate (and its `instant`) with the kit's or a maintained retry helper; the `deny_ignore` entry goes with it | Almanac, next turn |
| Theming, spacing, destructive-confirm dialog | With Kenny in the kp-themes session (prompt handed over 2026-09-06): layout utilities or a Bootstrap base, the theme-revert report on almanac.kp-soft.dev, a real dialog for `data-kp-confirm` | kp-themes session |
