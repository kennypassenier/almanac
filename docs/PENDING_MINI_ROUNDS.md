...
| Item | What | Who |
|---|---|---|
| CF-7 measurement (chassis-rs) | **Closed 2026-09-06 20:40** — Kenny logged in from Chrome on almanac.kp-soft.dev and deleted `almanac-test`; recorded as closed in chassis-rs `docs/PENDING_MINI_ROUNDS.md` (checked 2026-09-26). This row was stale | done |
...
| D5 · scaffold files | **done in 4.0.2** (chassis 1.7.0): kit CI/hooks/deny/Dockerfile/deploy from `chassis sync --write`; AR13 and M8 in `.claude/hooks/gates.project.sh`; `.chassis.toml` carries CT 112's measured `state_dir`/`env_file`/`latch_env` and three reviewed `deny_ignore` advisories (rsa Marvin via jsonwebtoken, `instant`, `backoff`) | done |
| `backoff` unmaintained (RUSTSEC-2025-0012) | **Done 2026-09-26** (`2d6d9aa`): the Calendar client retries through `backon` (already in the tree via chassis), same ladder (500 ms, ×1.5, jitter, one-minute budget); both `deny_ignore` entries for `backoff` and `instant` removed, `cargo deny check advisories` ok. Unreleased until 4.0.6 | done |
| Theming, spacing, destructive-confirm dialog | **Delivered by kp-themes** (checked 2026-09-26): the confirmation dialog landed there as W1 (`b45d2aad`), and the kit's `clients.html` marks its destructive buttons `data-kp-confirm`; almanac takes it through the kit's layout. Nothing left on almanac's side | done |
| `trusted_proxies` empty | Startup warning since 3.0.0: behind Traefik every client shares the proxy's IP; set `ALMANAC_TRUSTED_PROXIES` in the unit/env — announced to the homelab. **Still open, measured 2026-09-26:** the warning is in CT 112's journal at the last start (2026-09-10 20:22) and the unit's environment carries no `ALMANAC_TRUSTED_PROXIES` | Homelab Rust session |
| CF-12, Almanac's half (chassis-rs) | **Measured 2026-09-26.** Landed with the chassis 2.0.0 bump (`9bee012`, 2026-09-10): `tests/common` pins `ALMANAC_TOKEN` through `extra_env` instead of reading the harness's own token back. Now held as a check: `spawn_kit_in` asserts `app.token() == TOKEN`, seen failing against `KEY` first. Reported to chassis-rs through the project coordinator | done |
| latch push, still open | Checked 2026-09-26: CT 112's clone (`/opt/almanac/.latch/repo`) still shows `M almanac/dev/.env.enc`; GitHub's last `almanac/dev` push is `9f42305` (2026-08-29). The WSL machine now holds a PAT and latch, so Claude can carry it over — waits on Kenny's go in the form | Kenny (go) |
