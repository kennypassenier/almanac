...
- **Scaffold synced to chassis 1.8.0** (kit batch 3): `chassis::testing::TestApp`
  replaces the hand-written `tests/common` harness, wrapping the kit's own
  in-process start/login/issue/shutdown around Almanac's `AppState`
  assembly and profile/token-store fixtures. `App::vocabulary("source",
  "sources")` replaces `clients_label("Sources")`, so every kit-rendered
  sentence — not only the page heading — says "source". The standalone
  "Reload profiles from disk" form on `/calendars` is now a `SectionAction`
  on the status page's Sources section, with the busy-label and
  arm-before-act treatment every other kit button has. `docs/KIT.md`
  (kit-owned, generated by `chassis sync --write`) replaces the hand-kept
  self-update descriptions in `ARCHITECTURE_REFERENCE.md` and
  `OPERATIONS_RUNBOOK.md`, which had also drifted (a retired capture-store
  behaviour was still described as current). kp-themes 5.0.0 (25 themes,
  `topo`/`tazhib`/`nishiki` renamed to `forest`/`lapis`/`woodblock`,
  destructive confirmations as a native `<dialog>`) needed no template
  changes — Almanac's dashboard has been on the kit's own layout since the
  3.0.0 migration.

### Added

- **`chassis clients issue`** documented in `docs/USER_GUIDE.md` as a
